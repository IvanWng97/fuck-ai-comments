...
#[test]
fn trailing_line_prettier_ignore_does_not_target_the_next_node() {
    let source = concat!(
        "function render() {\n",
        "  oldCall(); // prettier-ignore\n",
        "  newCall();\n",
    ...
    assert!(
        findings
            .iter()
            .any(|finding| finding.rule == "comment-policy/function-comment-budget"),
        "a trailing line comment cannot direct Prettier at the next node: {findings:#?}"
    );
}

#[test]
fn inline_block_prettier_ignore_targets_a_same_line_node() {
    let source = concat!(
        "function render() {\n",
        "  oldCall(); /* prettier-ignore */ newCall();\n",
        "  // ordinary explanation\n",
        "  finish();\n",
        "}\n",
    );
    ...
    assert!(
        findings.is_empty(),
        "an inline block comment targets the same-line node after it: {findings:#?}"
    );
}

#[test]
fn jsx_wrapper_line_cannot_replace_the_comment_end_line() {
    let source = concat!(
        "const Card = () => (\n",
        "  <main>\n",
    ...
    assert!(
        ...
        "a JSX wrapper ending beside the target cannot make an earlier trailing comment operational: {findings:#?}"
    );
}
...
fn jsx_line_start_prettier_ignore_targets_across_blank_lines() {
    ...
    assert!(
        ...
        "a line-start JSX directive targets the next node across blank lines: {findings:#?}"
    );
}
